...
代价：回写给平台的响应帧里 ``type`` 也跟着变成了 ``event`` 而不是原来的 ``card``。

这一点已经真机验证过，不影响平台的交互处理。2026-08-31 在一个自建的免费团队
租户（应用 CRM Assistant，lark-oapi 1.7.3，长连接模式）上跑通了完整链路：发消息
收到事件 → 机器人回带表单的卡片 → 点击表单的提交按钮 → 客户端正常弹出 toast，
没有出现 ``200340``。也就是说平台确实按 ``message_id`` 关联请求与响应，帧头的
``type`` 只是回显，改写它不改变平台对这次交互的处理。

这个结论只对 1.7.x 的行为负责。升级 lark-oapi 之后重新跑一次
``scripts/ws_smoke.py``（点一下卡片按钮，看有没有 toast）再确认。
"""
