...
这些用例只能证明帧头被正确改写。补丁的另一半 —— 响应帧的 ``type`` 跟着变成
``event`` 之后平台还认不认 —— 只能真机验，已于 2026-08-31 在 lark-oapi 1.7.3
上验过：点卡片表单的提交按钮正常弹 toast，没有 ``200340``。升级 SDK 后用
``scripts/ws_smoke.py`` 重跑那一遍。
"""
