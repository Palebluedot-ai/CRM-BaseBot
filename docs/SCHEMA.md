...
**日期按业务时区**：看板里的交易日期是新加坡的日历日，导入时写成 `BUSINESS_TIMEZONE` 那一天的零点，先删后写也按这个时区取日期（2026-09-14 定的）。按 UTC 算的话，界面里手工填的日期是新加坡零点，会被算成前一天，重导时删不掉它。换算收在 `domain/dates.py`，种子数据用同一约定。
